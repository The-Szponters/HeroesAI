...
sf::Color resolve_palette_entry(int i, std::uint8_t r, std::uint8_t g, std::uint8_t b) {
    switch (i) {
        ...
        case 2:  return sf::Color(0, 0, 0, 64);
        case 3:  return sf::Color(0, 0, 0, 128);
        case 4:  return sf::Color(0, 0, 0, 128);
        case 5:  return sf::Color(0, 0, 0, 128);
        case 6:  return sf::Color(0, 0, 0, 160);
        case 7:  return sf::Color(0, 0, 0, 192);
        default: return sf::Color(r, g, b, 255);
    }
}
...
DefResource DefParser::parse_file(const std::filesystem::path& filepath) const {
    ...
    for (std::uint32_t block = 0; block < header.num_blocks; ++block) {
        ...
        for (std::uint32_t f = 0; f < num_frames; ++f) {
            char name[14] = {};
            if (!file.read(name, 13)) throw std::runtime_error("Unexpected EOF in frame name");
        }
        ...
        for (std::uint32_t f = 0; f < num_frames; ++f) {
            ...
            if (fmt == 1 && crop_w > 0 && crop_h > 0 && full_w > 0 && full_h > 0) {
                ...
                for (std::uint32_t y = 0; y < crop_h; ++y) {
                    ...
                    if (dy < 0 || dy >= static_cast<std::int32_t>(full_h)) continue;
                    for (std::uint32_t x = 0; x < crop_w; ++x) {
                        const std::int32_t dx = lmargin + static_cast<std::int32_t>(x);
                        if (dx < 0 || dx >= static_cast<std::int32_t>(full_w)) continue;
                        const sf::Color& c = resource.palette[indexed[y * crop_w + x]];
                        const std::size_t idx = (static_cast<std::size_t>(dy) * full_w + static_cast<std::size_t>(dx)) * 4u;
                        rgba[idx+0] = c.r; rgba[idx+1] = c.g;
                        rgba[idx+2] = c.b; rgba[idx+3] = c.a;
                    }
                }
    ...
    auto compute_anchor = [](const std::vector<DefFrame>& frames,
        ...
        for (const DefFrame& f : frames) {
            ...
            if (bottom > bottom_max) bottom_max = bottom;
            cx_sum += static_cast<std::uint64_t>(f.offset_x) + f.width / 2u;
            ++cx_count;
        }
        if (cx_count > 0) out_x = static_cast<std::uint32_t>(cx_sum / cx_count);
        out_y = bottom_max;
    };
    ...
    if (resource.feet_y == 0) {
        for (const auto& [_, frames] : resource.groups) {
            ...
            if (fy > resource.feet_y) { resource.feet_x = fx; resource.feet_y = fy; }
        }
    }
