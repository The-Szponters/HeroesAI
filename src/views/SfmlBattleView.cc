...
void SfmlBattleView::load_action_bar_assets( ){
    ...
    add(ActionKind::AUTO_COMBAT, "autocombat_icon.def", x ); x -= kIconW + kPad;
...
void SfmlBattleView::refresh_expanded_highlights( ){
    ...
    for( const AttackOriginHex& origin : attack_origin_highlights ){
        ...
        if( origin.has_tail ){
            expanded_highlights[make_hex_key(origin.tail_q, origin.tail_r)] = HighlightType::ATTACK_ORIGIN;
        }
    }
    ...
    if( hover_destination_highlight.has_value() ){
        ...
        if( hover.has_tail ){
            expanded_highlights[make_hex_key(hover.tail_q, hover.tail_r)] = HighlightType::HOVER_DESTINATION;
        }
    }
    ...
    if( active_unit_highlight.has_value() ){
        ...
        if( active.size == 2 ){
            ...
            expanded_highlights[make_hex_key(active.q + tail_dq, active.r)] = HighlightType::ACTIVE_UNIT;
        }
    }
}
...
void SfmlBattleView::update_visual_events(sf::Time dt ){
    ...
    if( finished ){
        ...
        while( !visual_events.empty() && visual_events.front().type == VisualEvent::Type::COMMIT_RENDER_DATA ){
            process_visual_event_start( );
            process_visual_event_finish( );
            visual_events.pop_front( );
        }
