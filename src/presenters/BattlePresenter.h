...
private:
    ...
    static std::optional<views::UnitRenderData> find_unit(const std::vector<views::UnitRenderData>& units,
                                                         std::uint64_t id );
    ...
    static std::vector<views::IBattleView::AttackOriginHex> dedupe_attack_origins(
        const std::vector<views::IBattleView::AttackOriginHex>& origins );
    const std::vector<views::IBattleView::AttackOriginHex>* get_cached_attack_origins_for_target(
        const models::Unit& target) const;
    const models::Hex* resolve_move_head_destination(const models::Unit& unit,
    ...
    models::Hex* find_attack_approach(const models::Unit& attacker, const models::Hex& target_hex,
    ...
    std::unordered_map<std::uint64_t, std::vector<views::IBattleView::AttackOriginHex>> cached_attack_origins_by_target;
