...
UnitRenderData
make_unit_render_data( const GameManager& model, const Unit& unit, int q, int r, bool is_corpse ) {
    UnitRenderData data;
...
void BattlePresenter::on_hex_clicked( int q, int r, bool ) {
    ...
    if ( clicked_hex->has_unit( ) && clicked_hex->get_unit( ).get( ) != active_unit ) {
        ...
        try {
            if ( will_shoot ) {
                ...
                const PickedApproach picked =
                    pick_attack_approach_for_cursor( *active_unit,
                                                     *clicked_hex,
                                                     static_cast<float>( last_cursor_px ),
                                                     static_cast<float>( last_cursor_py ) );
                Hex* approach = picked.approach;
                if ( approach == nullptr ) {
                ...
                try {
                    ...
                    for ( const Hex* h : chain ) {
                        if ( h != nullptr )
                            intended_path_for_attack.emplace_back( h->get_q( ), h->get_r( ) );
                    }
                } catch ( const std::exception& ) {}
                model.attack( *active_unit, *target, *approach );
                view.show_message( intended_path_for_attack.size( ) >= 2 ? "Move + Attack!"
                                                                         : "Attack!" );
            }
        ...
        const bool defender_died = after_def.has_value( ) && after_def->is_corpse &&
        ...
        const bool attacker_died = after_att.has_value( ) && after_att->is_corpse &&
        ...
        if ( will_shoot ) {
            ...
            view.queue_projectile_animation( attacker_id,
                                             after_def->q,
                                             after_def->r,
                                             active_unit->get_projectile_asset( ),
                                             0.4f );
    ...
    if ( move_head_hex != nullptr ) {
        ...
        try {
            ...
            for ( const Hex* h : chain ) {
                if ( h != nullptr )
                    intended_path.emplace_back( h->get_q( ), h->get_r( ) );
            }
        } catch ( const std::exception& ) {}

        try {
            Hex& move_head =
                model.get_board( ).get_hex( move_head_hex->get_q( ),
                                            move_head_hex->get_r( ),
                                            -move_head_hex->get_q( ) - move_head_hex->get_r( ) );
...
void BattlePresenter::on_mouse_hover( int pixel_x, int pixel_y, bool shift_held ) {
    ...
    const auto [q, r] =
        pixel_to_hex( static_cast<float>( pixel_x ), static_cast<float>( pixel_y ) );
    const int s = -q - r;
    ...
    if ( shift_held && hovered_hex->has_unit( ) ) {
        ...
        view.set_cursor_style( CursorStyle::QUESTION_MARK, pixel_x, pixel_y );
        return;
    }
    ...
    if ( const Hex* move_head_hex = resolve_move_head_destination( *active_unit, *hovered_hex );
        ...
        const sf::Vector2f hovered_px =
            hex_to_pixel( move_head_hex->get_q( ), move_head_hex->get_r( ) );
        ...
        if ( active_unit->get_size( ) == 2 ) {
            ...
            view.set_hover_destination_highlight(
                move_head_hex->get_q( ), move_head_hex->get_r( ), false, 0, 0 );
        }
    } else {
        view.clear_hover_destination_highlight( );
    }
    ...
    if ( ! is_enemy ) {
        ...
        const bool reachable =
            ( resolve_move_head_destination( *active_unit, *hovered_hex ) != nullptr );
        CursorStyle empty_style = CursorStyle::NOT_AVAILABLE;
        if ( reachable ) {
            empty_style = active_unit->is_teleporter_unit( ) ? CursorStyle::FLY_MOVE
    ...
    if ( active_unit->is_ranged( ) && active_unit->get_ammo( ) > 0 ) {
        ...
        if ( hovered_unit != nullptr && model.will_shoot( *active_unit, *hovered_unit ) ) {
            ...
            view.set_cursor_style( dist > 10 ? CursorStyle::BROKEN_ARROW : CursorStyle::RANGE_SHOOT,
                                   pixel_x,
                                   pixel_y );
            ...
            for ( const Hex* hex : cached_destinations ) {
                ...
                view.highlight_hex(
                    hex->get_q( ) + predicted_tail_dq, hex->get_r( ), HighlightType::WALKABLE );
                view.highlight_hex(
                    hex->get_q( ) - predicted_tail_dq, hex->get_r( ), HighlightType::WALKABLE );
            }
            return;
    ...
    const PickedApproach picked = pick_attack_approach_for_cursor(
        *active_unit, *hovered_hex, static_cast<float>( pixel_x ), static_cast<float>( pixel_y ) );
    Hex* approach_hex = picked.approach;

    if ( approach_hex == nullptr ) {
        view.clear_attack_origin_highlights( );
        view.set_cursor_style( CursorStyle::NOT_AVAILABLE, pixel_x, pixel_y );
        return;
    }

    if ( ! picked.all_origins.empty( ) ) {
        view.set_attack_origin_highlights( picked.all_origins );
    } else {
        view.clear_attack_origin_highlights( );
    }

    {
        ...
        view.set_hover_destination_highlight(
            approach_hex->get_q( ), approach_hex->get_r( ), has_tail, tail_q, tail_r );
        ...
        for ( const Hex* hex : cached_destinations ) {
            ...
            if ( active_unit->get_size( ) != 2 )
                continue;
            const int predicted_tail_dq = active_unit->is_facing_left( ) ? 1 : -1;
            view.highlight_hex(
                hex->get_q( ) + predicted_tail_dq, hex->get_r( ), HighlightType::WALKABLE );
            view.highlight_hex(
                hex->get_q( ) - predicted_tail_dq, hex->get_r( ), HighlightType::WALKABLE );
    ...
    int attack_head_q = approach_hex->get_q( );
    int attack_head_r = approach_hex->get_r( );
    ...
    std::vector<std::pair<int, int>> target_body{
        { hovered_hex->get_q( ), hovered_hex->get_r( ) } };
...
void BattlePresenter::refresh_ui_for_active_unit( ) {
    ...
    view.update_hud(
        active_unit->get_name( ), active_unit->get_count( ), active_unit->get_health_left( ) );
    ...
    view.highlight_hex( active_unit->get_q( ), active_unit->get_r( ), HighlightType::ACTIVE_UNIT );
    if ( active_unit->get_size( ) == 2 ) {
    ...
    for ( const Hex* h : cached_destinations ) {
        if ( h != nullptr )
            cached_destinations_set.insert( make_hex_key( h->get_q( ), h->get_r( ) ) );
    }
    ...
    for ( Hex* dest : destinations ) {
        ...
        if ( path.size( ) >= 2 ) {
            ...
            if ( final_px.x < penult_px.x - kFlipDeadZone )
                facing_left = true;
            else if ( final_px.x > penult_px.x + kFlipDeadZone )
                facing_left = false;
        }
    ...
    for ( const Hex& hex : grid ) {
        ...
        if ( target == active_unit )
            continue;
        if ( hex.get_q( ) != target->get_q( ) || hex.get_r( ) != target->get_r( ) )
            continue;
        if ( ! model.are_enemies( *active_unit, *target ) )
            continue;
    ...
    for ( Unit* candidate : model.peek_next_round_order( ) ) {
        ...
        if ( candidate->get_count( ) <= 0 )
            continue;
        if ( ! model.are_enemies( *active_unit, *candidate ) )
            continue;
        if ( ! model.will_shoot( *active_unit, *candidate ) )
            continue;
        ...
        for ( const auto& [oq, orr] : origins ) {
            for ( int i = 0; i < 6; ++i ) {
                ...
                try {
                    ...
                    if ( ! nhex.has_unit( ) )
                        continue;
                    if ( nhex.get_unit( ).get( ) == active_unit )
                        continue;
                    if ( ! model.are_enemies( *active_unit, *nhex.get_unit( ) ) )
                        continue;
...
void BattlePresenter::show_unit_range_preview( const Unit& unit ) {
    ...
    view.clear_hover_destination_highlight( );
    view.clear_attack_origin_highlights( );
    highlight_unit_body( unit, HighlightType::ACTIVE_UNIT );
    ...
    for ( Hex* dest : destinations ) {
        ...
        if ( unit.get_size( ) == 2 ) {
            view.highlight_hex(
                dest->get_q( ) + unit_tail_dq, dest->get_r( ), HighlightType::WALKABLE );
        }
    }
    ...
    for ( Unit* candidate : model.peek_next_round_order( ) ) {
        ...
        if ( candidate->get_count( ) <= 0 )
            continue;
        if ( ! model.are_enemies( unit, *candidate ) )
            continue;
        ...
        if ( ! can_hit && model.will_shoot( unit, *candidate ) )
            can_hit = true;
...
BattlePresenter::dedupe_attack_origins( const std::vector<IBattleView::AttackOriginHex>& origins ) {
    std::vector<IBattleView::AttackOriginHex> out;
...
BattlePresenter::get_cached_attack_origins_for_target( const Unit& target ) const {
...
const Hex*
BattlePresenter::resolve_move_head_destination( const Unit& unit,
                                                const Hex& clicked_or_hovered_hex ) const {
    if ( is_destination_cached( clicked_or_hovered_hex.get_q( ),
                                clicked_or_hovered_hex.get_r( ) ) ) {
    ...
    for ( const int tail_dq : { -1, 1 } ) {
        ...
        if ( ! is_destination_cached( head_q, head_r ) )
            continue;
        try {
...
std::vector<UnitRenderData> BattlePresenter::build_render_data_snapshot( ) const {
    ...
    for ( const Hex& hex : grid ) {
        if ( hex.has_unit( ) ) {
            ...
            if ( hex.get_q( ) != unit->get_q( ) || hex.get_r( ) != unit->get_r( ) )
                continue;
            units.push_back(
                make_unit_render_data( model, *unit, hex.get_q( ), hex.get_r( ), false ) );
        }
        ...
        for ( const std::weak_ptr<Unit>& dead_weak : hex.get_dead_units( ) ) {
            if ( const std::shared_ptr<Unit> dead = dead_weak.lock( ) ) {
                ...
                units.push_back(
                    make_unit_render_data( model, *dead, dead->get_q( ), dead->get_r( ), true ) );
            }
        }
    }
...
void BattlePresenter::queue_move_visual_along_path( std::uint64_t unit_id,
    ...
    for ( Unit* u : model.get_unit_queue_in_round( ) ) {
        if ( u == nullptr || make_unit_id( u ) != unit_id )
            continue;
        if ( ! u->ignores_path_blockers( ) )
            break;
        ...
        const float duration_seconds =
            kSecondsPerHex * static_cast<float>( std::max( 1, hex_dist ) );
...
void BattlePresenter::queue_move_visual_if_needed( std::uint64_t unit_id,
    ...
    if ( ! before_unit.has_value( ) || ! after_unit.has_value( ) )
        return;
    if ( before_unit->q == after_unit->q && before_unit->r == after_unit->r )
        return;
    ...
    auto queue_single_segment = [&]( int from_q, int from_r, int to_q, int to_r ) {
        ...
        constexpr float kSecondsPerHex = 0.08f;
        const float duration_seconds =
            kSecondsPerHex * static_cast<float>( std::max( 1, hex_distance ) );
        view.queue_move_animation( unit_id, from_q, from_r, to_q, to_r, duration_seconds );
    };
        ...
        try {
            ...
            if ( chain.size( ) >= 2 ) {
                for ( std::size_t i = 1; i < chain.size( ); ++i ) {
                    queue_single_segment( chain[i - 1]->get_q( ),
                                          chain[i - 1]->get_r( ),
                                          chain[i]->get_q( ),
                                          chain[i]->get_r( ) );
                }
...
sf::Vector2f BattlePresenter::hex_to_pixel( int q, int r ) const {
    const float px = kGridOriginX +
                     kHexRadius * ( std::sqrt( 3.0f ) *
                                    ( static_cast<float>( q ) + static_cast<float>( r ) * 0.5f ) );
    const float py = kGridOriginY + kHexRadius * ( 1.5f * static_cast<float>( r ) );
    return { px, py };
}

Hex* BattlePresenter::find_attack_approach( const Unit& attacker,
                                            const Hex& target_hex,
                                            float pixel_x,
                                            float pixel_y ) const {
    const std::vector<Hex*>& reachable =
        cached_destinations.empty( ) ? ( const_cast<BattlePresenter*>( this )->cached_destinations =
    ...
    for ( Hex* candidate : reachable ) {
        ...
        if ( attacker.get_size( ) == 2 ) {
            attacker_body.emplace_back( candidate->get_q( ) + attacker_tail_dq,
                                        candidate->get_r( ) );
        }
...
BattlePresenter::PickedApproach BattlePresenter::pick_attack_approach_for_cursor(
    const Unit& attacker, const Hex& hovered_hex, float pixel_x, float pixel_y ) const {
    PickedApproach picked;
    if ( ! hovered_hex.has_unit( ) )
        return picked;
    Unit* target = hovered_hex.get_unit( ).get( );
    if ( target == nullptr || target == &attacker )
        return picked;
    if ( ! model.are_enemies( attacker, *target ) )
        return picked;

    picked.directly_adjacent = model.can_attack( attacker, hovered_hex );

    if ( const auto* cached = get_cached_attack_origins_for_target( *target ); cached != nullptr ) {
        picked.all_origins = *cached;
    }
    ...
    if ( picked.directly_adjacent ) {
        IBattleView::AttackOriginHex self_origin;
        self_origin.q = attacker.get_q( );
        self_origin.r = attacker.get_r( );
        if ( attacker.get_size( ) == 2 ) {
            const int tail_dq = attacker.is_facing_left( ) ? 1 : -1;
            self_origin.has_tail = true;
            self_origin.tail_q = self_origin.q + tail_dq;
            self_origin.tail_r = self_origin.r;
        }
        const bool present = std::any_of(
            picked.all_origins.begin( ), picked.all_origins.end( ), [&]( const auto& o ) {
                return o.q == self_origin.q && o.r == self_origin.r;
            } );
        if ( ! present )
            picked.all_origins.push_back( self_origin );
    }

    if ( picked.all_origins.empty( ) ) {
        if ( ! picked.directly_adjacent ) {
            picked.approach = find_attack_approach( attacker, hovered_hex, pixel_x, pixel_y );
        }
        return picked;
    }
    ...
    if ( target->get_size( ) == 2 ) {
        const int tdq = target->is_facing_left( ) ? 1 : -1;
        const sf::Vector2f tail = hex_to_pixel( target->get_q( ) + tdq, target->get_r( ) );
    ...
    for ( const IBattleView::AttackOriginHex& origin : picked.all_origins ) {
    ...
    if ( best != nullptr ) {
        picked.origin = *best;
        try {
            picked.approach = &model.get_board( ).get_hex( best->q, best->r, -best->q - best->r );
        } catch ( const std::out_of_range& ) {}
    }
    return picked;
}
