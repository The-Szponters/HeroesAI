    ...
    if( clicked_hex->has_unit() && clicked_hex->get_unit().get() != active_unit ){
        ...
        try {
            if( will_shoot ){
                ...
                view.show_message(intended_path_for_attack.size() >= 2 ? "Move + Attack!" : "Attack!" );
            }
...
void BattlePresenter::on_mouse_hover(int pixel_x, int pixel_y, bool shift_held ){
    ...
    if( active_unit->is_ranged() && active_unit->get_ammo() > 0 ){
        ...
        if( hovered_unit != nullptr && model.will_shoot(*active_unit, *hovered_unit) ){
            ...
            view.set_cursor_style(
                dist > 10 ? CursorStyle::BROKEN_ARROW : CursorStyle::RANGE_SHOOT,
                pixel_x, pixel_y );
    ...
    const PickedApproach picked = pick_attack_approach_for_cursor(
        *active_unit, *hovered_hex,
        static_cast<float>(pixel_x), static_cast<float>(pixel_y) );
...
void BattlePresenter::refresh_ui_for_active_unit( ){
    ...
    if( active_unit->get_size() == 2 ){
        view.highlight_hex(active_unit->get_q() + active_tail_dq,
                           active_unit->get_r(), HighlightType::ACTIVE_UNIT );
    }
...
BattlePresenter::PickedApproach BattlePresenter::pick_attack_approach_for_cursor(
    ...
    if( !hovered_hex.has_unit()) return picked;
    Unit* target = hovered_hex.get_unit().get( );
    if( target == nullptr || target == &attacker) return picked;
    if( !model.are_enemies(attacker, *target)) return picked;
    ...
    if( picked.directly_adjacent ){
        ...
        const bool present = std::any_of(picked.all_origins.begin(), picked.all_origins.end(),
            [&](const auto& o ){ return o.q == self_origin.q && o.r == self_origin.r; } );
        if( !present) picked.all_origins.push_back(self_origin );
    }
...
CursorStyle BattlePresenter::direction_to_cursor(float angle_deg) const {
    ...
    if( a < 30.0f || a >= 330.0f) return CursorStyle::SWORD_E;
    if( a < 90.0f)                return CursorStyle::SWORD_SE;
    if( a < 150.0f)               return CursorStyle::SWORD_SW;
    if( a < 210.0f)               return CursorStyle::SWORD_W;
    if( a < 270.0f)               return CursorStyle::SWORD_NW;
